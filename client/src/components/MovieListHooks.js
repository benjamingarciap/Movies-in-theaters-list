...
const MovieList = (props) => {
  ...
  useEffect(()=>{
    //get all movies 
    const numOfMoviesOnView = 21
    axios.get(`http://localhost:5000/api/movies?pageNo=${activeStep}&size=${numOfMoviesOnView}`)
    .then(resFromApi => {
  ...
  function handleNext() {
    ...
    axios.get(`http://localhost:5000/api/movies?pageNo=${activeStep+1}&size=${numOfMoviesOnView}`)
    .then(resFromApi => {
      setValue({ listOfMovies:resFromApi.data.message })
    })
    .catch(err=>{console.log("this error while fetching movies from the api", err)})
